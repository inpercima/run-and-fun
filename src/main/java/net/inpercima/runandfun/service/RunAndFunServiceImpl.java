...
public class RunAndFunServiceImpl implements RunAndFunService {
    ...
    public String getAccessToken(final String code, final String clientId, final String clientSecret,
        ...
        return handleResponse(openPostRequest(TOKEN_URL, query));
    }

    private HttpURLConnection openPostRequest(final String url, final String query)
            throws UnsupportedEncodingException, IOException {
        HttpURLConnection connection = (HttpURLConnection) new URL(url).openConnection();
        connection.setRequestMethod(METHOD_POST);
        connection.setDoOutput(true);
        connection.setRequestProperty(CONTENT_TYPE, FORM_URLENCODED);
